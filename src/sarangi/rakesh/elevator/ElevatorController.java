...
public class ElevatorController implements IElevatorController, ElevatorEventListener {
    ...
    private ArrayList<LinkedList<Integer>> passengers = null;
    ...
    public LinkedList<Integer> getPassengers(int floor) {
        if((floor<0) || (floor >= numFloors )  ) throw new IllegalArgumentException();
        return passengers.get(floor);
    }

    public void setPassengers(LinkedList<Integer> pssgrs, int floor) {
    ...
    public void pickup(int floor, boolean direction) {
        if((floor<0) || (floor >= numFloors )) throw new IllegalArgumentException();
        // shuffling the order of elevators in the case where most of the elevators are on the same floors and they are picked up at the same time from different floors
    ...
    public void onStopped(Object sender) {
        ...
        LinkedList<Integer> psQueue = getPassengers(((Elevator)sender).getCurrentFloor());
